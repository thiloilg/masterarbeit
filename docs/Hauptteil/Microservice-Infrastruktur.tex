...
Folgende Domains würden für die drei Server verwendet:

\begin{enumerate}
    \item Versionsverwaltung und CI/CD
    \begin{enumerate}
        \item gitlab.thiloilg.com
        \item registry.thiloilg.com
    \end{enumerate}
    \item Testphase
    \begin{enumerate}
        \item testing-stage.org
        \item api.testing-stage.org
        \item delivery.testing-stage.org
    \end{enumerate}
    \item JSON Ressourcen von AJAX Anfragen
    \begin{enumerate}
            \item blicc.org
            \item api.blicc.org
            \item delivery.blicc.org
            \item monitor.blicc.org
        \end{enumerate}
\end{enumerate}

Die beiden Subdomains "gitlab" und "registry" verweisen auf die Gitlab CE und die Docker Registry.
Die Domain "testing-stage.org" wird für die Integrationstests und zur Qualitätssicherung verwendet.
Die Domain "blicc.org" dient als Hauptdomain und wird in der Produktion verwendet. Der Name Blicc
wurde zur representation des Produktes gewählt. Bei einer Verwendung der Anwendung als White-Label-Produkt wird der Name durch den der zu repräsentierenden Firma ersetzt. Der Name Blicc ist eine
veränderte Form des deutschen Worts "Blick". Der Name wurde daher verändert, da die Verfügbarkeit
des deutschen Wortes "Blick" in den meisten Domainvariationen nicht mehr erhältlich war und zudem
überteuert angeboten wurde.
