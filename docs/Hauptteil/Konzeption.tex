...
\section{Entwurf}
\label{sec:entwurf}
Der Entwurf ist in drei Abschnitte unterteilt. In \Cref{subsec:trennungvonlogic} handelt von
der Trennung der Logic innerhalb des Systems. Die Trennung der Logic ist die wichtigste
Designentscheidung und somit an erster Stelle. In \Cref{subsec:wasgehoertindiedatenbank}
wird klar definiert, welche Ressourcen in der Datenbank anzufinden sind. In \Cref{subsec:diagrammanordnungsverfahren}
...
Handshake rüstet sich die Verbindung auf das WebSocket-Protokol auf. Die WebSocket-Verbindung
...
die Services komplett unabhängig skalier- und auslieferbar. Die Abhängigkeit der einzelnen
Services mit dem Frontend ist natürlich immernoch gegen. Diese Abhängigkeit muss man
allerdings in Kauf nehmen.

Ähnlich wie bei dem rwhuDIOA 
...
\subsection{Was gehört in die Datenbank?}
