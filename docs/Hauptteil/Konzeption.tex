...
Machbarkeitsnachweis \footnote{In der Businesswelt spricht man hier oftmals von einem POC, also Proof of Concept}
...
Als nächstes stellt sich die Frage nach der maximalen Anzahl an möglichen Stufen innerhalb einer angefragten Datenstruktur.
\footnote{Die Anzahl der Stufen beschreibt hier die Tiefe eines JSON-Objektes.}
Wenn man hierbei das ummantelnde Array einer Antwort bestehend aus mehreren Objekten als eine eigene Stufe zählt,
kommt man auf maximal drei Stufen. Auch hier sollte allerdings klar sein, dass sich dies um Laufe der Zeit noch ändern kann.
Somit ist auch hier auf eine Softwarelösung zu setzen, die flexibel auf eine größere Anzahl an Stufen in der Hierarchie der
durch Datenstruktur reagiert.
