...
werden vier Aspekte der Anforderungsanalyse an die Software erörtert. Als
erstes geht es um die Anforderungen an die Gestaltung der Software. Hierbei liegt der Fokus auf
der grafischen Oberfläche. Als zweites werden die Anforderungen an die Funktionalität der
Software diskutiert. Hierbei werden mögliche Vorgehensweisen der Benutzer betrachtet, anhand derer
dann Anforderungen an die Funktionalität der Software gestellt werden. Als drittes werden die 
Anforderungen an die Flexibilität der Software gestellt. Im Unterschied zu den Anforderungen
an die Funktionalität liegt hier ein besonderer Fokus auf der Vielfalt bestimmter Funktionalitäten.
Dabei stellt sich die Frage, wie flexibel die Software in Bezug auf die zu analysierenden Daten
und derer Visualisierung sein muss. Als viertes werden die Anforderungen an die Codebasis gestellt.
Hier geht es um die bedeutsame Qualitäten der Software. Es stellt sich die Frage, wie es mit den
Anforderungen an die Weiterverwendbarkeit sowie die Veränderbarkeit der Software aussieht.

Nach der Anforderungsanalyse gilt es die aktuelle Lage des Marktes zu analysieren. Tools im Bereich der
Geschäftsanalytik gibt es viele. \cite{WikiBISoftware} Was zeichnen diese Softwarelösungen aus? Mit welchen
Merkmalen kann man sich von vorhandenen Lösungen bewusst unterscheiden? Diese Fragen werden in dem Abschnitt
"Auswertung vorhandener Software" behandelt.

Der Abschnitt vier und fünf der Konzeption beschäftigt sich mit der Analyse zweier für die Arbeit
relevanter Technologien. Als erstes wird hier die Frage der Art der
Frontendapplikation behandelt. Dabei werden folgende Fragen beantwortet: Was ist eine
Progressive Web App und macht diese im Kontext der Benutzeranforderungen sinn?
Was sind Vor- und Nachteile gegenüber nativer Apps? Wie sieht es mit der
unterstützung der Plattformen aus? Als zweites werden folgende Fragen zur 

Wie könnte die Serverinfrastruktur aussehen? Was für Komponenten beinhaltet sie? Welche Eigenschaften
stellt die Infrastruktur bereit? Mit was für einer Verarbeitungsmenge muss man rechnen? Wie sieht
...
in fünf Rubriken unterteilt. Als erstes werden die Anforderungen an die Gestaltung der Anwendung
gestellt. Hier geht es primär um das Aussehen sowie die Benutzerfreundlichkeit
der Frontendanwendung. Als zweites werden die Anforderungen an die Funktionalität der
Software analysiert. Was sind typische Benutzerhandlungen, die die Software ermöglichen
sollte? Als drittes werden die Anforderungen zur Flexibilität der Datenquellen analysiert. Mit was
für Datensätzen wird die Software in Zukunft arbeiten? Als viertes
werden die Datenvisualisierungsarten analysiert. Welche Arten von Diagrammen und
Schaubildern benötigen die Benutzer, um aus ihren Daten brauchbare Informationen zu ziehen?
...
Was sind die Anforderungen an die Flexibilität der Software? Die Analyse der angeforderten Flexibilität
der Software ist in zwei Bereiche unterteilt. Im ersten Bereich geht es um die Flexibilität im Umgang mit
den Daten. Hier stellt sich die Frage, wie flexibel der Anschluss an die Datenquellen gestaltet werden soll?
Im zweiten Bereich geht es um die Flexibilität der Visualisierungsmöglichkeiten. Hier werden die Anforderungen
an die Flexibilität der Visualisierung der Daten anhand von Fallbeispielen erörtert.
