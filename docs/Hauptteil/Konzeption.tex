...
\section{Entwurf}
\label{sec:entwurf}
Der Entwurf ist in zwei Abschnitte unterteilt. In \Cref{subsec:trennungvonlogic} handelt von
der Trennung der Logic innerhalb des Systems. Die Trennung der Logic ist die wichtigste
Designentscheidung und somit an erster Stelle. In \Cref{subsec:diagrammanordnungsverfahren}
wird ein intuitives Verfahren vorgestellt, das die agile Erstellung von Dashboards
ermöglicht.

\subsection{Trennung von Logic}
...
können diese auch wieder gelöscht werden. Dabei sind die zuvor beschriebenen Verfahren einfach rückläufig.

Die Anordnung selbst soll als Baumstruktur im JSON-Format dargestellt werden. Um Änderungen in der Baumstruktur
schneller zu erkennen, muss die Baumstruktur klein gehalten werden. Daher soll die Baumstruktur nur UUIDs
als Referenzen auf die einzelnen Diagramme beinhalten. Somit kann unterschieden werden, ob sich etwas
an der Anordnung oder den Einstellungen der einzelnen Diagramme verändert hat.
