...
Alle weiteren Instanzen werden nach Auslastung auf die Nodes verteilt. In Abbildung \ref{figure:dockerswarmcluster}
ist die Content Delivery (Nginx) und der Resource Management Service (API) auf zwei Instanzen skaliert.
Der Data Delivery Service (Delivery) ist durch vier Instanzen repräsentiert.
...
Die Arbeit verwendet als Webserver Nginx. Für die kompression der statischen
...
beiden Kompressionsarten wird in \Cref{sec:uebertragung} dargestellt und ausgewertet.
...
Immer wieder vorkommende Funktionalitäten wurden in 28 React-Hooks ausgelagert. So besitzt die Frontendanwendung
der Arbeit beispielsweise einen Hook für die Anordnung der bereits im Dashboard existierenden Diagramme, einen für
die Positionierung neuer Diagramme und einen für das Zeichen der Kacheln, die beim Drag and Drop erscheinen.
...
der Anwendung und der Web Storage API synchronisiert. Verändert der Nutzer als beispielsweise die Sprache in einem
...
stellt eine \code{publish} sowie eine \code{subscribe} Funktion zur Verfügung. Die \code{useApiEndpoint}
dient gleichzeitig als Middleware zwischen der Anwendung und dem Backend. So werden hier HTTP-Statuscodes
abgefangen, um so bestimmte Aktionen durchzuführen. So wird im Falle eines abgelaufenen Authentifizierungstokens,
ein neues beantragt. Daraufhin wird die Fehlgeschlagene Anfrage erneut ausgeführt. 
