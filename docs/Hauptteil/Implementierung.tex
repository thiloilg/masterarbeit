...
In diesem Kapitel geht es um die Umsetzung des in \Cref{chap:konzept} erarbeiteten
...
folgt gegliedert: In \Cref{sec:infrastruktur} geht es um
die für die Entwicklung, die Testphase sowie die in Produktion laufende Anwendung
benötigten Server. Der Aufbau der Gesamtanwendung wird in \Cref{sec:gesamtsysten} behandelt.
Daraufhin wird in \Cref{sec:projektaufbau} der Projektaufbau geschildert.
In \Cref{sec:pipeline} wird die Implementierung der stetigen
...
findet in \Cref{sec:frontendanwendung} statt. In \Cref{sec:plugins} geht es um die
Implementierung der für die Dashboards benötigten Plugins, welche die Daten
in Diagrammen visualisieren. Des Weiteren wird in \Cref{sec:resourcemanagementservice}
die testgetriebene Entwicklung der für die Benutzerverwaltung benötigte REST-API dargestellt.
Zuletzt wird in \Cref{sec:datadeliveryservice} die Implementierung
...
Insgesamt werden vier Linux-Server verwendet: Einen für die Versionsverwaltung
...
direkt über IP-Adressen ansprechbar.} Die Domain \code{blicc.org} dient als
...
den "Resource Management Service"; die Subdomain \code{delivery} auf den "Data Delivery Service".
...
\code{docker} \code{swarm} \code{init} auf dem Manager-Server
...
Komponenten. \Cref{subsec:systemuebersicht} gibt einen groben Überblick über
...
werden in \Cref{subsec:reverseproxyundloadbalancer} behandelt. In
\Cref{subsec:inhaltsauslieferung} wird die Inhaltsauslieferung
...
\Cref{subsec:wahlderdatenbankarten} befasst sich mit der für die einzelnen
...
Das Orchestrierungssystem Docker Swarm (Swarm) übernimmt die Aufgabe des Load Balancers. Swarm verwendet
als Lastverteilungsverfahren Round\hyp{}Robin, eine rotierende Verteilung der eingehenden Anfragen auf
...
Zur Inhaltsauslieferung wird der Webserver Nginx verwendet.
...
den Nginx Docker-Container installiert und in der \code{nginx.conf} aktiviert werden.
...
in einem Shell-Skript der Befehl \code{service nginx status} aufgerufen. Ist das Resultat positiv, wird
das Skript mit \code{0} beendet, andernfalls mit \code{1}. Bei den eigenen Services werden \code{/health-check}
...
erwähnt, das CSS-Framework Bootstrap verwendet. Im Rahmen der Arbeit wurde sich bewusst gegen
...
Bis auf die Bibliotheken \code{jmespath.js} und \code{qrcode} wurde die komplette Funktionalität der Frontend-Anwendung
selbst implementiert. Die Zustandsverwaltung wurde mit der Context API von React realisiert.
Immer wieder vorkommende Funktionalitäten wurden in React-Hooks\footnote{React-Hooks bieten die Möglichkeit,
Zustand in funktionalen Komponenten zu Verwalten, was bis Ende Oktober 2018 nur bei Klassenkomponenten möglich war.\cite{MothershipFunctionalComponents}}
ausgelagert. Diese Hooks werden in einem \code{common} Ordner zusammen mit
oft benutzten React-Komponenten ausgelagert. Die Routen der Frontend-Anwendung
besitzten nur für die Seiten, die Ressourcen anzeigen (Dashboards, Charts und DataSources), den von REST bekannten Aufbau der URL-Pfade. Die Pfade
wurden möglichst benutzerfreundlich benannt. So ist der URL-Pfad der Loginseite beispielsweise \code{/login}.
Aus Suchmaschinenoptimierungsgründen und der Benutzerfreundlichkeit wegen verändert sich der Titel sowie die Beschreibung je nach
...
Des Weiteren wurden externe Schnittstellen in Hooks verpackt, um so die Schnittstelle von der Anwendung
...
Zum Zeichnen der farbigen Bereiche, die beim Ziehen der
Typen "Chart" oder "Data Source" erscheinen,
wird ein HTML\hyp{}Canvas, der über dem jeweiligen Plugin positioniert wird, verwendet.
Hierfür wird der \code{useDrawQuad} Hook verwendet. Zur Errechnung der Position, über die der Typ
losgelassen wurde, werden die Canvas\hyp{}Eckpunkte verwendet, um den zuletzt fokussierten Bereich zu bestimmen.
Dies geschieht mithilfe des \code{useSelectAction} Hooks. Die Umsetzung orientiert sich an dem
in \Cref{subsec:diagrammanordnungsverfahren} beschriebenen Verfahren. Der komplexeste Hook ist der
\code{useArrangement} Hook. Anhand der aus dem \code{useSelectAction} Hook resultierenden Aktion wird
mithilfe des \code{useArrangement} Hooks das neue Plugin in die Baumstruktur, welche die Positionierung
...
den \code{useArrangement} Hook, wurden vor der eigentlichen Implementierung zahlreiche Modultests definiert.
Somit konnte sichergestellt werden, dass die Operationen auf der Datenstruktur keine Fehler aufweisen.

Die zwei bedeutsamsten Hooks sind die zwei folgenden Hooks: \code{useApiEndpoint} und \code{useDeliveryEndpoint}. Sie ermöglichen
...
stellt eine \code{publish} sowie eine \code{subscribe} Funktion zur Verfügung. Der \code{useApiEndpoint} Hook
dient gleichzeitig als Middleware zwischen der Anwendung und dem Backend. So werden hier HTTP-Statuscodes
abgefangen, um bestimmte Aktionen durchzuführen. Im Falle eines abgelaufenen Authentifizierungstokens,
wird ein neues beantragt. Daraufhin wird die fehlgeschlagene Anfrage erneut ausgeführt.
...
für das anfängliche Laden der Frontend-Anwendung über einen CDN. Dies tritt nur ein,
...
ein JSON-Objekt, das den Aufbau und die Einstellungen eines Dashboards darstellt.\footnote{Ein
...
Kommen die Plugins vor den zu visualisierenden Daten an, abonnieren diese über 
...
Zum Erstellen sowie als Sicherheitsabfrage vor der Löschung von Ressourcen werden Dialogfenster verwendet.
Hierfür wird ein \code{useModal} Hook bereitgestellt. Ein Beispiel befindet sich in
...
eine \code{ProtectedRoute}, welche den Benutzer umleitet, falls dieser keine Rechte für den
angefragten URL-Pfad besitzt.\footnote{Hierbei ist anzumerken, dass die eigentliche Rechteverwaltung durch
das Backend abgesichert ist.} Außerdem beinhaltet die Frontend-Anwendung Eingabevalidierungen der Formulare
...
ein zusätzliches Dialogfenster angezeigt, dass die Anmeldedaten abfragt. Um die initiale Zugriffssicherheit zu erhöhen,
bietet die Anwendung eine Zwei-Faktor-Authentisierung.
...
die Seiten sehr viel Zeit in Anspruch nehmen. Hierfür wurde eine Suchfunktion implementiert. Die Suchfunktion befindet sich
in der oberen Navigationsleiste. Durch einen Zeichenvergleich des Titels können die passenden Ressourcen schneller gefunden
...
im Anhang unter "Screenshots" in \Cref{figure:sucheundbearbeitungdesbenutzerprofils}.
...
aufgerufen, wird ein Feld im SessionStorage gesetzt, das die Information über
den installierten Zustand speichert. Mithilfe eines \code{useInstalled} Hooks kann innerhalb
...
Da sich die Seitenleiste, über welche die Ressourcen im nicht-installierten Zustand erreichbar sind, bei mobilen
...
Im Service Worker wurden zwei Zwischenspeicherstrategien implementiert,
um die Performanz zu steigern und eine bedingte Verwendung der Anwendung
ohne Internetverbindung zu ermöglichen.\footnote{Zum jetztigen Stand
werden keine AJAX-Anfragen im Service Worker abgefangen. Ohne eine bestehende Internetverbindung,
können daher nur statische Inhalte abgerufen werden.} Hierfür werden die statischen Dateien in zwei
...
Liegt die Anfrage im Zwischenspeicher, wird diese zurückgegeben. Andernfalls wird eine Anfrage über das
...
dem Cache geladen. Besteht keine Internetverbindung und die Datei liegt nicht im Zwischenspeicher, wird
...
\section{Plugins}
\label{sec:plugins}
Für die Implementation der Plugins wurde mit \code{Storybook}\footnote{https://storybook.js.org/}, einem Werkzeug
...
von dem Gesamtprojekt ermöglichen. In einer \code{.env} Datei im Wurzelverzeichnis des Git-Submodules "Plugins"
müssen die Domain des "Resource Management Services" sowie die E-Mail-Adresse und das Password eines Benutzers mit
der Rechtegruppe "Entwickler" oder "Admin" gesetzt sein. Mit einem einfachen \code{yarn deploy} können so
...
Die Einstellungen werden in der Ressource der jeweiligen Dashboards persistiert.
Somit lassen sich Einstellungen wie die letzten vom Nutzer verwendeten Filteroptionen speichern.
Zur Veranschaulichung wurde mithilfe von Chart.js ein Liniendiagramm, ein Kuchendiagramm und
...
wurde die Laufzeitumgebung Node.js verwendet. Als Programmiergerüst wird in der Arbeit Koa.js, der Nachfolger
von Express.js, verwendet.\footnote{https://koajs.com/} Verglichen zu Express.js ist Koa.js eher als eine Ansammlung
...
wurde sich im Rahmen der Arbeit für Koa.js entschieden. Als Programmiersprache verwendet
der "Resource Management Service" TypeScript.
...
Die Hashes, die die Passwörter repräsentieren, werden mithilfe der Bibliothek \code{bcryptjs} generiert.
...
der Template-Sprache Handlebars\footnote{https://handlebarsjs.com/} erstellt. Das Testing-Framework Jest\footnote{https://jestjs.io/} wurde zur Realisierung der Integrationstests
...
\subsection{Authentifizierungsverfahren und Rechteverwaltung}
\label{subsec:authentifizierungsverfahrenundrechteverwaltung}
Für das Authentifizierungsverfahren werden JSON Web Token (JWT) verwendet. Das Entscheidungskriterium hierfür war,
...
in den Cookies mit den Flags \code{samesite=strict},\footnote{Als Schutz gegen CSRF-Attacken} \code{secure} und \code{httponly}
...
in dem JWT steht, spart sich der "Resource Management Service" eine Abfrage gegen die Datenbank. Die Middleware
zur Rechteverwaltung sichert nur den Zugriff der Benutzergruppen auf die Routen.
...
der Daten Go-JMESPath.\footnote{https://github.com/jmespath/go-jmespath} Im "Data Delivery Services"
wird Golang verwendet, da bereits bei der Gestaltung der Programmiersprache Wert auf
...
wird der Syntax-Befehl \code{go}, gefolgt von einer zu parallelisierenden Funktion, benutzt.\cite{GoroutinesExample}}
...
wird der Golang interne HTTP-Server des \code{net/http} Programmpakets verwendet. Der "Data Delivery Service"
